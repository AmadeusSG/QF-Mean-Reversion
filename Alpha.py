...
class Model:
	...
	def __init__(self):
		...
		self._num_stocks = int(input("Input the number of stocks : "))
	...
	def select_samples(self):
		...
		for day in self._daily_quotes:
			if len(self._daily_quotes[day]) > self._num_stocks:
				self._daily_quotes[day].sort(key=lambda x: x[7], reverse=True)
				self._daily_quotes[day] = self._daily_quotes[day][:self._num_stocks]
